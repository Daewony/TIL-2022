...
**인라인 요소:** 텍스트 처럼, 지속적으로 가로로 배치되는 애들

**블럭 요소:** 한 줄에 하나씩 배치되는 애들

item에 display: inline-block 

텍스트에서 한칸 줄바꿈하면 띄어쓰기로 판단한다

텍스트사이에 아무리 엔터, 띄어쓰기를 해도 한칸만 적용된다(제한적)

HTML의 목표는 어떤 컨텐츠 구조를 잡는 역할이다(표현 X)
...
텍스트의 하단 행간을 위해 아랫 공간이 생긴다

베이스 라인으로 인해 아랫 공간이 생긴다

⇒ 인라인 블럭은 다 좋은데 텍스트의 성질때문에 뛰어쓰기 공간 또는 하단 공간이 생긴다

**없애는 방법은?**

편법) font-size:0, div 한줄로 표현하기(→ 팁: 사이에 주석을 넣는 방법)

좌우에 마진 마이너스 값을 넣기(가변적이라 완벽하지 않음)

깔끔하게 없애는 방법은 없다, 인라인 블럭의 고유한 특성이기 때문이다

**하단 공간 없애기**

vertical-align: top

width 없이 padding으로도 가능하다

---

float

item에게 float:left 적용 → 가로배치됨 + 한칸 띄기 문제 없음

그러나 다른 문제 발생

float 된 요소의 부모의 높이가 0이 되어버린다(백그라운드 컬러가 깔릴 수 있는 공간이 없어서 사라지는 것을 볼 수 있다)

해결방법

부모에게 overflow: hidden 적용

왜 float 된 요소의 부모의 높이가 0이 되어버릴까?

float는 본질적으로 가로배치를 하기 위한 속성이 아니기 때문이다

float의 원래 목적은 백과사전 사진 처럼 레이아웃을 형성 하는 것이다
...
/* auto는 컨텐츠 크기 만큼 너비 설정됨 */

가운데 정렬: justify-content: center

호환성 문제때문에 위 2가지 기술을 배우는 것이다
}

## 1128 {
    # ****생각처럼 동작하지 않는 height: 100%****

Q. div 에서 클래스 선택자를 CSS로 사진을 추가했는데 왜 안나올까?

A. div가 비어있기 때문이다. 아무것도 컨텐츠가 없기 때문이다

컨텐츠가 없으면 백그라운드 이미지가 깔 공간자체가 없었던 것이 된다

```css
CSS 디폴트 밸류
/* cover의 부모인 body의 너비 만큼 결정된다(width 값을 지정하지 않았을때) */
width: auto; /* 요소의 부모 크기 기준으로 가득찬다 */
/* cover의 자식 높이 기준으로 자동 조절된다(height값 지정하지 않았을때) */
height: auto; /* 요소의 자식 기준으로 자동 조절된다 */

너비는 꽉차지만 높이는 텍스트(내용물) 만큼의 높이로 설정됨

% 유닛의 내용을 잘 알아야한다

height: 100% /* 부모 height를 받겠다, cover의 부모 body의 height값을 이어 받음 */

body의 height도 위 CSS 디폴트 밸류 조건에 따라서 변화가 일어나기때문이다
조건에 따라서
body의 height는 자식인 cover의 높이만큼 설정된다

서로를 기준을 삼고있는 웃긴 상황

html,body}
	height: 100%; 하면 꼼수로 가능하다
{

CSS3 방법
height: 100vh; (viewport height, 화면 높이)

background-position: center; 사진 가운데 정렬
background-size: cover; 창크기를 늘리고 줄였을때 창크기에 맞게끔 조절이 된다
